...
% fig_s1_2_fixed_L2_varied_L1_ave_plot.m

% Plots average OHC region or BM vibrations for f2 = 9 kHz, L2 = 60 dB SPL,
% and both f1 and L1 varied. 
% Average data are plotted vs. L1 for each f2/f1 ratio 
% (~1.07-1.67 in 0.1 steps).

% All individual data are loaded from 'ohc_fixed_L2_varied_L1.mat' 
% or 'bm_fixed_L2_varied_L1.mat', which include vibration magnitudes (dB re 1 nm RMS) 
% and phases (cycles).

% Set the location to 'ohc' or 'bm' to plot the data from that measurement
% location.

close all;clear;clc;

%% Set measurement location here:
location = 'ohc'; % either 'ohc' or 'bm'

%% Load relevant data file
switch location 
    case 'ohc'
        load 'ohc_fixed_L2_varied_L1';
    case 'bm'
        load 'bm_fixed_L2_varied_L1';
end
...
for f_i = 1:f1N
    ...
    % Plot each component ('f1', 'f2', 'dp2f1_m1f2' and 'dp1f2_m1f1')
    for c_i = 1:componentN 
        ...
        % Average vibrations (in dB re 1 nm RMS)
        ...
        % Plot
        axes(ax1);
        plot(L1s, vib_magC_aveC, '-', 'LineWidth', 1.6, 'Color', clr); % average line
        errorbar(L1s, vib_magC_aveC, vib_magC_se, '-', 'LineWidth', 1.6, 'Color', clr); % average +/- SE
        plot(L1s, vib_magC_aveC, '-', 'LineWidth', 1.4, 'Color', clr, 'Marker', mrk, 'MarkerFaceColor', mrkClr,'MarkerSize', mrkSz); % average symbol  
    end
    %pause; 
    %close all;
end